...
def send_random_traffic(num_packets, interface, src_ip, dst_ip):
    ...
    src_mac= "CA:FE:CA:FE:CA:FE" #EX1 New Mac Address
    total_pkts = 0
    port = 1024
    for i in range(num_packets):
            data = randomword(512) #EX1 512B size
            p = Ether(dst=dst_mac,src=src_mac)/IP(dst=dst_ip,src=src_ip)
            p = p/TCP(sport= 5555, dport=port)/Raw(load=data) #EX1 New Source port, Now using TCP
